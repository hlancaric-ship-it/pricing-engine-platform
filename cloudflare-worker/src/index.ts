...
import {
    ...
    VIP_CART_JS,
    VIP_CATALOG_JS,
    VIP_CART_COUPON_LOCK_JS,
    VIP_REGISTRATION_HIDE_TYPES_JS,
} from './static-assets';
import { LICENSE_FINGERPRINT } from './licensing/build-info';
...
async function triggerGithubSync(
    ...
    try {
        const res = await fetch(
            'https://api.github.com/repos/hlancaric-ship-it/okfish-pricing-engine/dispatches',
            {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    event_type: 'shoptet-webhook',
                    client_payload: { event, eventInstance },
                }),
            },
        );
        if (!res.ok)
            console.error('[webhook] GitHub dispatch failed', res.status, await res.text());
    } catch (e) {
...
export default {
    // Thin wrapper around handleFetch: attaches the license fingerprint
    // header to every response and nothing else. All actual request
    // handling logic lives in handleFetch, untouched by this wrapper.
    async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
        const response = await handleFetch(request, env, ctx);
        const headers = new Headers(response.headers);
        headers.set('X-Engine-Build', LICENSE_FINGERPRINT);
    ...
    async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext) {
        ...
        ctx.waitUntil(
            runFeedGeneration(env, filename, version).catch((e) => {
...
async function handleFetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (path === '/v1/feed/generate' && request.method === 'POST') {
        ...
        if (statusData) {
            ...
            if (parsed.status === 'running') {
                return jsonResponse(
                    { error: 'Already running.', startedAt: parsed.startedAt },
                    429,
                );
            }
        }
    ...
    if (path === '/v1/import/chunk' && request.method === 'POST') {
        ...
        const body = (await request.json()) as {
            version: string;
            customers: Array<{ hash: string; discount: number }>;
        };
    ...
    if (path === '/v1/webhook/shoptet' && request.method === 'POST') {
        ...
        if (payload.event?.startsWith('product:') && !productCode) {
            console.warn(
                '[webhook] product event but could not find a `code` field in payload -- logging shape for debugging:',
        ...
        if (env.GITHUB_DISPATCH_TOKEN) {
            ctx.waitUntil(
                triggerGithubSync(
                    ...
                    payload.event,
                    productCode ?? payload.eventInstance,
                ),
            );
        } else {
            console.error(
                '[webhook] GITHUB_DISPATCH_TOKEN not configured -- signature verified but no sync triggered',
            );
        }
        return jsonResponse({ ok: true });
    }
    ...
    if (path === '/v1/products/import/chunk' && request.method === 'POST') {
        ...
        const body = (await request.json()) as {
            version: string;
            products: Array<{ code: string; row: CsvRow }>;
        };
    ...
    if (path.startsWith('/v1/price-cache/') && request.method === 'POST') {
        ...
        return jsonResponse({
            ok: true,
            pricelistId,
            updated: Object.keys(body.updates).length,
    ...
    if (path.startsWith('/v1/product-discount/') && request.method === 'GET') {
        ...
        const tier = parts[1];
        if (!code || !tier)
            return jsonResponse({ error: 'Usage: /v1/product-discount/:code/:tier' }, 400);
        ...
        const standardPrice = parseFloat(
            (row['standardPrice'] || row['price'] || '0').replace(',', '.'),
        );
        const finalPrice = tierResult.price;
        const discountPct =
            standardPrice > finalPrice ? Math.round((1 - finalPrice / standardPrice) * 100) : 0;
