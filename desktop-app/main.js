...
ipcMain.on('app-ready', () => {
    ...
    mainWindow.center();
    // Runs after the app is already usable, never blocking startup on a
    // network round-trip to GitHub -- an update check failing or being slow
    // must never be the reason the dashboard doesn't show up.
    checkForUpdates();
});
...
// Checks GitHub Releases on this repo (public, so no token needed) for a newer
// tagged build, downloads it silently in the background, and prompts to
// restart once it's ready. Means Pavol (or anyone else this app is ever sent
// to) never again has to manually re-run Setup.exe over an existing install --
// every future fix/feature just ships as a new tagged release.
// Only meaningful in a packaged build: a dev run via `electron .` has no
// app-update.yml (that file is generated by electron-builder) and would just
// throw on every check, so this is skipped entirely outside app.isPackaged.
function checkForUpdates() {
    if (!app.isPackaged) return;

    autoUpdater.on('update-available', (info) => log(`Nová verze appky (${info.version}) — stahuji na pozadí...`));
    autoUpdater.on('error', (err) => log(`CHYBA při kontrole aktualizace: ${err.message}`));
    autoUpdater.on('update-downloaded', async (info) => {
        log(`Aktualizace ${info.version} stažena.`);
        const { response } = await dialog.showMessageBox(mainWindow, {
            type: 'info',
            buttons: ['Restartovat teď', 'Později'],
            defaultId: 0,
            title: 'L-Code Pricing Engine',
            message: `Je k dispozici nová verze appky (${info.version}).`,
            detail: 'Appka se restartuje a aktualizace se dokončí sama.'
        });
        if (response === 0) autoUpdater.quitAndInstall();
    });

    autoUpdater.checkForUpdates().catch((err) => log(`CHYBA při kontrole aktualizace: ${err.message}`));
}
